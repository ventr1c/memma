...
# OpenAI key needed for embeddings (text-embedding-3-small) and QA/judge (gpt-4o-mini)
...
CONFIG_PATH="$EXPERIMENT_ROOT/configs/memory_toolkits/amem_locomo_claude_haiku.json"
...
RUN_DIR="$RESULTS_ROOT/amem_locomo_claude-haiku_${TIMESTAMP}"
TOKEN_COST_PREFIX="$RUN_DIR/token_cost_amem"
mkdir -p "$RUN_DIR"

echo "[INFO] Running A-MEM baseline (Claude)"
...
(
  cd "$RUN_DIR"
  conda run -n amem_env python "$TOOLKIT_DIR/memory_construction.py" \
...
(
  cd "$RUN_DIR"
  conda run -n amem_env python "$TOOLKIT_DIR/memory_search.py" \
    --memory-type "$MEMORY_TYPE" \
    --dataset-type "$DATASET_TYPE" \
    --dataset-path "$DATASET_PATH" \
    --config-path "$CONFIG_PATH" \
...
(
  cd "$RUN_DIR"
  conda run -n amem_env python "$TOOLKIT_DIR/memory_evaluation.py" \
...
echo "[DONE] A-MEM baseline (Claude) completed."
