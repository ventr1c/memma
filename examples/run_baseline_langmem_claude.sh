...
CONFIG_PATH="$EXPERIMENT_ROOT/configs/memory_toolkits/langmem_locomo_claude_haiku.json"
...
# TIMESTAMP=20260228_022504
RUN_DIR="$RESULTS_ROOT/langmem_locomo_claude-haiku_${TIMESTAMP}"
TOKEN_COST_PREFIX="$RUN_DIR/token_cost_langmem"
mkdir -p "$RUN_DIR"

echo "[INFO] Running LangMem baseline (Claude)"
...
(
  cd "$RUN_DIR"
  conda run -n langmem_env python "$TOOLKIT_DIR/memory_construction.py" \
...
(
  cd "$RUN_DIR"
  conda run -n langmem_env python "$TOOLKIT_DIR/memory_search.py" \
    --memory-type "$MEMORY_TYPE" \
    --dataset-type "$DATASET_TYPE" \
    --dataset-path "$DATASET_PATH" \
    --config-path "$CONFIG_PATH" \
...
(
  cd "$RUN_DIR"
  conda run -n langmem_env python "$TOOLKIT_DIR/memory_evaluation.py" \
...
echo "[DONE] LangMem baseline (Claude) completed."
