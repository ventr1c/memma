...
BUILD_MODEL="${BUILD_MODEL:-gpt-4o-mini}"
QA_MODEL="${QA_MODEL:-gpt-4o-mini}"
JUDGE_MODEL="${JUDGE_MODEL:-gpt-4o-mini}"
RETRIEVAL_MODE="${RETRIEVAL_MODE:-combined}"
TOTAL_LIMIT="${TOTAL_LIMIT:-10}"
LIMIT_PER_SPEAKER="${LIMIT_PER_SPEAKER:-5}"
...
JUDGE_BASE_URL="${JUDGE_BASE_URL:-$OPENAI_API_BASE}"
...
API_KEYS_CSV="${API_KEYS_CSV:-$OPENAI_API_KEY}"
IFS=',' read -r -a API_KEYS_ARRAY <<< "$API_KEYS_CSV"
if [[ "${#API_KEYS_ARRAY[@]}" -eq 0 ]]; then
  echo "[ERROR] No API keys available for build stage."
  exit 1
fi

MODEL_TAG=$(echo "$QA_MODEL" | tr -cs 'A-Za-z0-9._-' '-')
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
RUN_DIR="$RESULTS_ROOT/lightmem_official_openai_${MODEL_TAG}_${TIMESTAMP}"
...
echo "[INFO] Running LightMem official LoCoMo baseline (OpenAI)"
echo "[INFO] PYTHONPATH=$PYTHONPATH"
echo "[INFO] DATASET=$DATASET_PATH (total=$TOTAL, ratio=$RATIO, start=$START_IDX, end=$END_IDX)"
echo "[INFO] BUILD_MODEL=$BUILD_MODEL, QA_MODEL=$QA_MODEL, JUDGE_MODEL=$JUDGE_MODEL"
...
if [[ "${#VALID_BUILD_KEYS[@]}" -eq 0 ]]; then
  echo "[ERROR] No valid non-empty API keys after parsing API_KEYS_CSV."
  exit 1
fi
...
echo "[DONE] LightMem official baseline (OpenAI) completed."
