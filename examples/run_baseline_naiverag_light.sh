...
CONFIG_PATH="$EXPERIMENT_ROOT/configs/memory_toolkits/naiverag_locomo_gpt4omini.json"
...
RUN_DIR="$RESULTS_ROOT/naiverag_locomo_gpt4omini_${TIMESTAMP}"
TOKEN_COST_PREFIX="$RUN_DIR/token_cost_naiverag"
mkdir -p "$RUN_DIR"

echo "[INFO] Running NaiveRAG baseline"
...
(
  cd "$RUN_DIR"
  conda run -n rag_env python "$TOOLKIT_DIR/memory_construction.py" \
...
(
  cd "$RUN_DIR"
  conda run -n rag_env python "$TOOLKIT_DIR/memory_search.py" \
    --memory-type "$MEMORY_TYPE" \
    --dataset-type "$DATASET_TYPE" \
    --dataset-path "$DATASET_PATH" \
    --config-path "$CONFIG_PATH" \
...
(
  cd "$RUN_DIR"
  conda run -n rag_env python "$TOOLKIT_DIR/memory_evaluation.py" \
...
echo "[DONE] NaiveRAG baseline completed."
