#!/usr/bin/env python3
"""
A-Mem + Meta-Thinker Integration Script (MeMMA Self-Refine, Claude mixed 0314)

Uses A-Mem (ChromaDB + AgenticMemorySystem + evolution) for memory
construction/retrieval, with a mixed MemMA model split:
- Backbone for memory manager / construction meta guidance / query reasoning / meta-thinker:
  Claude Haiku 4.5 via Bedrock
- Answer agent + LLM judge: gpt-4o-mini
- Build-time self-refine answer/judge: gpt-4o-mini
...
class BedrockOpenAICompat:
    """Expose a boto3 Bedrock client through an OpenAI-like chat.completions API."""
...
class BedrockLLMClient:
    """Bedrock Converse client with the same interface as LLMClient."""
...
def _patch_amem_anthropic_for_bedrock() -> None:
    """Monkeypatch A-Mem's AnthropicController so Bedrock model IDs work transparently."""
    try:
        from lightmem.memory_toolkits.memories.layers.baselines.agentic_memory import llm_controller as amem_llm_controller
        import anthropic
    except Exception:
        return

    class BedrockAwareAnthropicController:
        ...
        def _strip_code_fences(text: str) -> str:
            text = text.strip()
            m = amem_llm_controller._CODE_FENCE_RE.match(text)
            return m.group(1).strip() if m else text

        def get_completion(self, prompt: str, response_format: dict = None, temperature: float = 0.7) -> str:
            system_content = "You must respond with a JSON object. Do not include any text outside the JSON."
    ...
    amem_llm_controller.AnthropicController = BedrockAwareAnthropicController


_patch_amem_anthropic_for_bedrock()
...
class AMemMetaThinkerEvaluator:
    ...
    def __init__(
        ...
        **_extra,
        ...
        self.meta_thinker_llm = _create_llm_client(
        ...
        self.logger.info(
            ...
            type(self.answer_llm).__name__,
            self.answer_llm.model,
        )
        self.logger.info(
        ...
        if HAS_REFLECTION:
            self.reflection_engine = ReflectionEngine(self.meta_thinker_llm, self.logger)
    ...
    def answer_question(
        ...
        result["answer"] = self.answer_llm.get_completion(prompt, temperature=0.0).strip()
        ...
        try:
            if LIGHTMEM_LLM_JUDGE_AVAILABLE and lightmem_llm_judge is not None:
                accuracy = lightmem_llm_judge(
                    ...
                    client_obj=self.answer_llm.client, model_name=self.answer_llm.model
                )
            else:
                accuracy = 0
    ...
    def evaluate_dataset(
        ...
        return {
            ...
            "answer_model": self.answer_llm.model,
    ...
    try:
        ...
        def _run_single_self_refine_job(job: Dict[str, Any]) -> Dict[str, Any]:
            ...
            return run_session_self_refinement(
                ...
                llm_client=runtime_view["llm_client"],
                self_refine_source=self_refine_source,
                session_qa_index=session_qa_index,
...
def main():
    ...
    parser.add_argument("--backbone_model", type=str, default="", help="Backbone model for A-Mem construction and Meta-Thinker")
    parser.add_argument("--answer_model", type=str, default="gpt-4o-mini", help="Answer generation and judge model")
    ...
    if args.build_memories:
        ...
        logger.info(f"Build-Time Self-Refine Answer/Judge Model: {answer_model}")
        ...
        construction_llm_client = None
        if args.enable_construction_meta_guidance:
            construction_llm_client = _create_llm_client(
        ...
        self_refine_llm_client = None
        if args.self_refine_source != "none":
            self_refine_llm_client = _create_llm_client(
                model=answer_model,
                region=args.region,
                api_key=api_key,
                base_url=args.base_url,
        ...
        amem_config_kwargs = dict(
            llm_model=backbone_model,
            llm_backend=args.llm_backend,
            embedder_provider=args.embedder_provider,
            retriever_model=args.retriever_model,
            evo_threshold=args.evo_threshold,
            api_key=None if (args.llm_backend == "anthropic" and _is_bedrock_model(backbone_model)) else api_key,
            base_url=args.base_url,
        )
        ...
        if args.self_refine_source != "none":
            self_refine_runtime = init_self_refine_runtime(
                llm_client=self_refine_llm_client,
        ...
        if effective_build_max_workers > 1 and len(samples) > 1:
            ...
            for sample in tqdm(samples, desc="Building memories"):
                ...
                result = build_memories_for_sample(
                    sample=sample,
                    llm_client=construction_llm_client if should_pass_llm_client else None,
                    logger=logger,
                    **build_kwargs_common,
                )
                results.append(result)
    ...
    results["backbone_model"] = backbone_model
    results["answer_model"] = answer_model
