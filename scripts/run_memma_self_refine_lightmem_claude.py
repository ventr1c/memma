...
try:
    ...
    retry_error = None
    if isinstance(e, ModuleNotFoundError) and getattr(e, "name", "") == "spacy":
        try:
            _install_spacy_shim()
            from retrievers import QdrantEntryLoader, VectorRetriever, format_related_memories
            from prompts import ANSWER_PROMPT, METADATA_GENERATE_PROMPT_locomo
            HAS_LIGHTMEM = True
            SPACY_SHIM_ACTIVE = True
            print(
                "Warning: spacy is missing in current env. "
                "Using lightweight shim for retriever import compatibility."
            )
    ...
    if not HAS_LIGHTMEM:
        final_error = retry_error or e
        print(f"Warning: Cannot import LightMem utilities: {type(final_error).__name__}: {final_error}")
...
def _build_memory_manager_config(llm_model: str, api_key: str, api_base_url: str = None, aws_region: str = None) -> dict:
    """Return the memory_manager config block, choosing provider based on model name."""
    if _is_bedrock_model(llm_model):
        return {
...
def get_lightmem_config(
    ...
    return {
        ...
        "text_summary": True,
        "memory_manager": _build_memory_manager_config(llm_model, api_key, api_base_url, aws_region),
...
class BedrockLLMClient:
    ...
    def get_completion(
        ...
        converse_msgs = [{"role": "user", "content": [{"text": prompt}]}]
...
def _create_llm_client(model: str, region: str = "us-west-2", api_key: str = None, base_url: str = None):
...
def init_self_refine_runtime(
    ...
    if not HAS_LIGHTMEM:
        raise RuntimeError("LightMem retriever is unavailable; cannot run self refinement.")
    ...
    if HAS_LIGHTMEM_EMBEDDER and embedding_model_path:
        try:
            ...
            runtime["retriever"] = VectorRetriever(embedder)
        except Exception as exc:
            logger.warning(f"Self-refine retriever init failed, fallback to entry slicing: {exc}")
    else:
        logger.warning("Self-refine retriever not initialized; using non-vector fallback retrieval")
    ...
    if requested_mode == "full_memma":
        try:
            evaluator = LightMemMetaThinkerEvaluator(
                ...
                retrieve_k=retrieve_k,
                qr_max_turns=3,
                enable_meta_thinker=True,
                retrieval_mode="combined",
                logger=logger,
            )
            if evaluator.retriever is None:
                logger.warning(
                    "Self-refine full_memma requested but evaluator retriever is unavailable; fallback to simple mode."
                )
                effective_mode = "simple"
            else:
                runtime["full_memma_evaluator"] = evaluator
        except Exception as exc:
            logger.warning(f"Self-refine full_memma init failed; fallback to simple mode: {exc}")
            effective_mode = "simple"
...
def _format_memories_by_speaker_for_refine(retrieved_entries: List[Dict[str, Any]]) -> tuple:
    ...
    if len(speaker_names) == 1:
        s1 = speaker_names[0]
        return s1, format_related_memories(speaker_groups[s1]), "Speaker 2", "No memories available."

    s1 = speaker_names[0]
    s2 = speaker_names[1]
    return (
        s1,
        format_related_memories(speaker_groups[s1]),
        s2,
        format_related_memories(speaker_groups[s2]),
    )
...
def _answer_question_for_self_refine(
    ...
    if retriever is not None:
        retrieved = retriever.retrieve(entries, question, limit=retrieve_k)
    else:
    ...
    for entry in retrieved:
        ...
        evidence.append(
            {
                "id": str(entry.get("id", "")),
                "speaker": str(payload.get("speaker_name", "Unknown")),
...
def _check_and_merge_fact(
    ...
    """Check if a proposed fact duplicates or complements an existing Qdrant entry.
    ...
    logger = logger or logging.getLogger(__name__)
    try:
        vec = lightmem.text_embedder.embed(fact)
    except Exception as exc:
        logger.warning(f"[SemanticDedup] embedding failed, falling back to INSERT: {exc}")
        return ("INSERT", None, None)

    try:
        hits = lightmem.embedding_retriever.search(vec, limit=3, return_full=True)
    except Exception as exc:
        logger.warning(f"[SemanticDedup] Qdrant search failed, falling back to INSERT: {exc}")
        return ("INSERT", None, None)
...
Decide: SKIP, MERGE, or INSERT."""
    ...
    if action == "MERGE":
        ...
        if not merged_fact or merge_idx < 0 or merge_idx >= len(candidates):
            ...
            return ("INSERT", None, None)
        target_id = str(candidates[merge_idx].get("id", ""))
        if not target_id:
    ...
    for action in actions:
        ...
        if llm_client is not None:
            ...
            if decision == "MERGE" and merged_fact and target_id:
                try:
                    new_vec = lightmem.text_embedder.embed(merged_fact)
                    existing = lightmem.embedding_retriever.get(target_id)
                    if existing:
                        existing_payload = existing.payload if hasattr(existing, "payload") else existing.get("payload", {})
                        updated_payload = dict(existing_payload)
                        updated_payload["memory"] = merged_fact
                        updated_payload["compressed_memory"] = merged_fact
                        lightmem.embedding_retriever.update(
                            target_id, vector=new_vec, payload=updated_payload,
                        )
                        dedup_stats["merge"] += 1
                        applied_action = dict(action)
        ...
        if HAS_MEMORY_ENTRY:
            entry = MemoryEntry(
                ...
                weekday=weekday,
                category="self_refine_fact",
                subcategory="",
                memory_class="",
                memory=fact,
                original_memory=fact,
                compressed_memory=fact,
                topic_id=None,
                topic_summary="",
                speaker_id=speaker_id,
                speaker_name=speaker_name,
            )
            lightmem.offline_update([entry])
        else:
            refine_turn = [
                {
                    "role": "user",
            ...
            lightmem.add_memory(
                prepare_messages_for_lightmem(refine_turn),
...
def run_session_self_refinement(
    ...
    entry_loader = runtime.get("entry_loader")
    if entry_loader is None:
        raise RuntimeError("Self-refine runtime missing entry_loader")
    ...
    entries_probe = entry_loader.load_entries(sample_id, with_vectors=True)
    if not entries_probe:
        if self_refine_skip_empty_entries:
            ...
            logger.warning(
                ...
                f"deferred: empty entries qa={len(questions)}"
            )
    ...
    for batch_start in range(0, len(questions), batch_size):
        ...
        # Frozen state evaluation for this batch.
        entries_before = entry_loader.load_entries(sample_id, with_vectors=True)
        if not entries_before:
            if self_refine_skip_empty_entries:
                skipped_batches += 1
                logger.warning(
                    f"[SelfRefine] sample={sample_id} session={session_idx} "
                    f"batch={batch_start // batch_size} skipped: empty entries"
                )
        ...
        # Skip intermediate global dedup -- the final offline_update_all_entries
        # in build_memories_for_sample handles consolidation in one pass.

        entries_after = entry_loader.load_entries(sample_id, with_vectors=True)
...
class LightMemMetaThinkerEvaluator:
    ...
    def __init__(
        ...
        if self.rewrite_rerank_strategy not in valid_rerank_strategies:
            self.logger.warning(
                "Unknown rewrite_rerank_strategy=%s, fallback to keep_new_prune_old",
                self.rewrite_rerank_strategy,
            )
            self.rewrite_rerank_strategy = "keep_new_prune_old"
        ...
        # Initialize Meta-Thinker with procedural memories (uses dedicated LLM if provided)
        self.meta_thinker = MetaThinkerAgent(
            self.meta_thinker_llm,
            self.logger,
    ...
    def _entry_brief(self, entry: Dict, max_len: int = 180) -> str:
        ...
        speaker = str(payload.get("speaker_name", "Unknown"))
        ts = str(payload.get("time_stamp", "")) or str(payload.get("timestamp", ""))
        memory = (
            payload.get("memory")
    ...
    def _llm_select_cap(
        ...
        try:
            raw = self.meta_thinker_llm.get_completion(
                prompt,
                temperature=0.0,
                max_tokens=500,
    ...
    def _rerank_by_original_question(
        ...
        if not unique_candidates:
            return []
        try:
            return self._thread_safe_retrieve(
                unique_candidates,
                question,
                limit=min(limit, len(unique_candidates)),
            )
        except Exception as e:
            self.logger.warning(f"Rerank failed, fallback to stable order: {e}")
...
def build_memories_for_sample(
    ...
    if (enable_meta_guidance or self_refine_source == "realtime") and llm_client is None:
    ...
    try:
        ...
        self_refine_deferred_sessions = 0
        self_refine_deferred_qa = 0
        ...
        for session_idx, (session, timestamp) in enumerate(session_pairs):
            ...
            entries_before_session = collection_entry_count(sample_id, qdrant_dir)
...
Provide construction guidance for the memory manager."""
                ...
                if enable_meta_guidance and guidance and METADATA_GENERATE_PROMPT_locomo:
                    ...
                    is_last_turn = (session_idx == len(session_pairs) - 1 and turn_idx == num_turns - 1)
                    lightmem.add_memory(
                        turn_messages,
                        dynamic_extraction_prompt,
                    ...
                    dynamic_extraction_prompt = METADATA_GENERATE_PROMPT_locomo
                    is_last_turn = (session_idx == len(session_pairs) - 1 and turn_idx == num_turns - 1)
                    lightmem.add_memory(
                        turn_messages,
                        dynamic_extraction_prompt,
            ...
            if self_refine_source != "none":
                ...
                entries_now = collection_entry_count(sample_id, qdrant_dir)
                if entries_now <= entries_before_session:
                    self_refine_deferred_sessions += 1
                    self_refine_deferred_qa += qa_estimate
                    logger.info(
                ...
                for pending_job in pending_self_refine_sessions:
                    ...
                    if bool(report.get("deferred_due_to_empty_entries")):
                        ...
                        continue
                    if int(pending_job.get("session_idx", -1)) < session_idx:
        ...
        if self_refine_source != "none" and pending_self_refine_sessions:
            ...
            if pending_self_refine_sessions:
                logger.warning(
                    ...
                    "pending_sessions=%d pending_qa_est=%d",
                    sample_id,
                    len(pending_self_refine_sessions),
                    pending_self_refine_qa_est,
                )
        ...
        if self_refine_source != "none":
            ...
            self_refine_summary = {
                "deferred_due_to_empty_entries_sessions": self_refine_deferred_sessions,
                "deferred_due_to_empty_entries_qa_est": self_refine_deferred_qa,
                ...
                "reports_count": len(self_refine_reports),
            ...
            logger.info(
                "  Self-Refine deferred(empty entries): sessions=%d qa_est=%d",
                self_refine_deferred_sessions,
                self_refine_deferred_qa,
            )
            logger.info(
            ...
            logger.info(
                "  Self-Refine totals: reports=%d qa=%d actions=%d noop=%d filtered_low_conf=%d "
                "speaker_amb_noop=%d invalid_span_noop=%d skipped_batches=%d",
                len(self_refine_reports),
                total_refine_qa,
...
def main():
    ...
    # Model settings
    parser.add_argument("--model", type=str, default="gpt-4o-mini", help="LLM model name (Bedrock model IDs auto-detected)")
    ...
    if args.build_memories:
        ...
        llm_client = _create_llm_client(
            model=args.model, region=args.region, api_key=api_key, base_url=args.base_url,
        )
        ...
        if effective_build_max_workers > 1 and len(samples) > 1:
            ...
            for sample in tqdm(samples, desc="Building memories"):
                should_pass_llm_client = (not args.disable_meta_thinker) or (args.self_refine_source == "realtime")
    ...
    # Create separate meta-thinker LLM if --meta_thinker_model is provided
    mt_llm = None
    if args.meta_thinker_model:
        mt_llm = _create_llm_client(
            model=args.meta_thinker_model, region=args.region,
            api_key=api_key, base_url=args.base_url,
        )
        logger.info(f"Meta-thinker LLM: {type(mt_llm).__name__} (model={args.meta_thinker_model})")
