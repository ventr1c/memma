...
class MultiAgentCycleEvaluator:
    ...
    def __init__(
        ...
        # Create agents
        self.memory_manager = MemoryManagerAgent(self.llm, self.logger)
        self.query_rewriter = QueryRewriterAgent(self.llm, self.logger, qr_mode=qr_mode)
        self.answer_agent = AnswerAgent(self.llm, self.logger)
        
        # Create Meta-Thinker if needed
        self.meta_thinker: Optional[MetaThinkerAgent] = None
        if meta_thinker_mode in ("vanilla", "reflection"):
            self.meta_thinker = MetaThinkerAgent(self.llm, self.logger)
